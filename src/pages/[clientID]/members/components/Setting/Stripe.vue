...
import {
  CheckStripeConnectConnectedDocument,
  DisconnectStripeConnectDocument,
  RequestStripeConnectDocument,
...
const {
  ...
  mutate: mutateCheck,
} = useMutation(CheckStripeConnectConnectedDocument)

const connected = ref(false)
onCheckDone(({ data }) => {
  connected.value = Boolean(data?.checkStripeConnectConnected)
  if (connected.value) {
    sendTrackUnchecked('tenant_member_stripe_initialized')
  }
})
onCheckError(() => {
...
async function setChannel() {
  channel = await getBroadcastChannel('stripe_connected')
...
function goBack() {
  return emit('updateStep', SetupStep.Default)
}
function onContinue() {
...
const {
  onDone: onConnectedDone,
  onError: onConnectedError,
  mutate: mutateConnected,
} = useMutation(RequestStripeConnectDocument)

onConnectedDone(({ data }) => {
  sendTrackUnchecked('tenant_member_stripe_initialized')
  window?.open(data?.requestStripeConnect, '_blank')
})
onConnectedError(() => {
  emit('error')
...
const {
  onDone: onDisconnectDone,
  onError: onDisconnectError,
  mutate: mutateDisconnect,
} = useMutation(DisconnectStripeConnectDocument)

onDisconnectDone(({ data }) => {
  connected.value = false
})
onDisconnectError(() => {
  emit('error')
...
<template>
  <div class="flex w-96 flex-col">
    <a v-if="!connected" href="#" class="stripe-connect dark w-48" @click.prevent="mutateConnected()">
      <span>Connect with Stripe</span>
    </a>
    <div v-else class="flex items-center justify-between">
      <div>
        <p class="text-heading pb-1 text-stone-800">Already connected to Stripe</p>
        <p class="text-body text-stone-500">
          Connected to
          <span class="font-bold text-emerald-600"> {{ workspaceStore.currentWorkspace?.name }}</span>
        </p>
      </div>
      <SpButton is-shadow class="bg-gray-200 px-4 py-3 hover:bg-gray-300" @click="mutateDisconnect">
        Disconnect
      </SpButton>
    </div>
    <div class="flex-1" />
